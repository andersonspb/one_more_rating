...
<pre>
rating_for(@my_rateable, :scope => curent_user.country.id)
</pre>
...
<i>max</i> Maximum rating value. 10 by default.


Example
<pre>
rating_for(@my_rateable, :scope => curent_user.country.id, :url => one_more_rating.ratings_path, :period => :week)
</pre>